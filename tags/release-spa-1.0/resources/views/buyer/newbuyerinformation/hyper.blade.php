...
<?php
/* Testbed for PiwikTracker: PHP Client for Piwik Tracker API */
// -- Matomo Tracking API init -- 


/*
CartController::complete_successful_transaction(
	360,
	'1513134514',
	22.00,
	'1712131110030431');
...
$pt->setNewVisitorId();
$pt->setURL('https://opensupermall.com/cart/'.time());

$pt->addEcommerceItem("3533","Piyo Piyo","Babyware",15,1);
$pt->addEcommerceItem("4743","Cleansing Gel 50g","Beauty",5.88,1);

$pt->doTrackEcommerceOrder(time(),54.43,42.44,4.43,7.54,0);

$pt->doTrackPageView('Hello Matomo World: '.time());
$pt = null;
...
<div class="col-sm-12"> 
	<div class="row">
		  <h2 style="text-align:left; float:left">Hyper</h2>
		  <h4 style="text-align:right;float:right">
		  <span id="ow_product_count">{{count($ow_product)}}</span>
		  <small>Hyper  @if(count($ow_product)!=1)products @else product @endif
	 found</small></h4>
	<p> </p>
		  <div class="col-sm-12">
			<div class="row">
			...
			{{-- Ends --}}
							...
							?>							
						     <div class="col-md-4">
                              ...
                              <td class="text-right">{{$currentCurrency}} {{number_format($price * $product->owarehouse_moq,2)}}</td>
  						          		</tr>
                            <tr>
                              <td>MOQ/location</td>
                              <td class="text-center">{{$product->owarehouse_moqperpax}}</td>
							  <input type="hidden" value="{{$product->owarehouse_moqperpax}}" id="moqperpax" />
                              <td class="text-right">{{$currentCurrency}} {{number_format($price * $product->owarehouse_moqperpax,2)}}</td>
                            </tr>
