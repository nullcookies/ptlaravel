...
class EmailController extends Controller
{
    ...
    public function confirmEmail($email, $role_id)
    {
        ...
        */
        try {
            ...
            // Send mail
            Mail::send('emails.general.confirm_email', ['user' => $user, 'confirm_url' => $url, 'name' => $user->first_name, 'status' => $status,'hours'=>$globalvars->email_confirmation_duration], function ($m) use ($user, $email) {
    ...
    public function do_confirmEmail($token, $email, $purpose, $user_role_id)
    {
        ...
        if (!is_null($trecord)) {
            ...
            $hours = $c->diffInHours();
            if ($trecord->status == "expired" or $trecord->count > 0 or $hours > $globalvars->email_confirmation_duration) {
                $message_type = "error";
        ...
        try {
            switch ($user_role_id) {
                case 2:
                    ...
                    break;
                case 3:
					$modal = Merchant::where('user_id', $user->id)->update(['emailconfirm' => true]);
                    break;
                case 11:
    ...
    public function merchantOnboard($email)
    {
        try {
           ...
           // $url = url() . '/c/verify/' . $token . '/' . $email . '/' . $purpose . '/' . 3 . '/confirm';			
            $url = url();			
            Mail::send('emails.merchant.onboard', ['user' => $user, 'confirm_url'=>$url, 'hours' => $globalvars->email_confirmation_duration], function ($m) use ($email, $user) {
