...
public function stockreport($id=null){
    try{
        ...
        $report_data = Stockreport::with('report_products','checker','creator','checker_company','creator_company','fairlocation','report_products.product')
                ->first();
        return view('do.report', compact('report_data','selluser'));
    }catch(\Exception $e){
        \Illuminate\Support\Facades\Log::info('Error @ '.$e->getLine().' file '.$e->getFile().' '.$e->getMessage());
        throw new CustomException($e);        
    }
}
