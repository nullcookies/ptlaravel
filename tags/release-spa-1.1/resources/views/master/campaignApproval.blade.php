				...
				<tr>
					...
					<td id="status_column" class="text-center">
						<span id="status_column_text">
							@if($campaign->status == 'sendpending')
								Send Pending
							@else
					...
					<td>
						<div class="action_buttons">
							...
							} else if($campaign->status == 'sendpending'){
								$approve->getPendingSend();
							}
							echo $approve->view;
							?>
						</div>
...
$(document).ready(function () {
	...
	$(document).delegate( '.send_campaign', "click",function (event) {
		...
		var obj = $(this);
		//obj.html("Sending...");
		campaings[0]=$(this).attr('current_role_id');
		$.ajax({
			type: "POST",
			data: {campaings: campaings},
			url: "/seller/member/admin/send_campaign",
			dataType: 'json',
			success: function (data) {
				toastr.info("Campign successfully sent!");
				//obj.html("Send");
			},
	...
	$(document).delegate( '.send_campign_c', "click",function (event) {
		$("#save_remarks").addClass('send_campaign');
		$("#save_remarks").attr('current_role_id',$(this).attr('current_role_id'));
	/*	var campaings={};
		var obj = $(this);
		//obj.html("Sending...");
		campaings[0]=$(this).attr('current_role_id');
