                ...
                <tbody id="new-terminal"> 
                    ...
                    @if(count($products) > 0)
                        ...
                        <tr>
                            ...
                            <td class="text-center">{{$data->nproduct_id}}</td> 
                            <td class="text-left"><img style="object-fit:cover;" width="30"  height="30" src="{{url()}}/images/product/{{$data->id}}/thumb/{{$data->thumb_photo}}">&nbsp;{{$data->name}}</td>                
                            <td class="text-center"><a href="#" data-toggle="modal" data-target="" onclick="listrecipe('{{$data->id}}')">0</a></td>
                        </tr>
                    @endforeach
                    @endif               
                </tbody>
    ...
    <script>
        ...
        function setskudatatable(listproducts) {
            ...
            jQuery.each( listproducts, function( key, listproduct ) {
                skutablerow+=     `<tr class='highlite1' onclick="getproduct(`+listproduct.id+`,'`+listproduct.thumb_photo+`',`+listproduct.price+`,'`+listproduct.name+`')">
        ...
        function getproduct (product_id,thumb_photo,price,name,quantity=1,online=true){
            ...
            $.ajax({
                ...
                success: function(response){
                    ...
                    table.row.add([ 1, product_id, image + name, measuredata,unit ]).node().id = "rawmaterial_"+product_id;

                    table.draw();
                    $(".raw-datatable_td table tbody tr td:nth-child(2)").addClass("text-center");
                    $('#listrecipeModal').modal('show');
                    $('#skumodel').modal('hide');
                }
            });            
