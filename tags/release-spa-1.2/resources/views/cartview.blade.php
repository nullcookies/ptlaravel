...
<meta property="og:image"
	content="{{asset('/images/product/'.$product->id)}}/{{$product->photo_1 }}" />
				...
				?>
			  	...
			  	@foreach($merchants as $product)
					...
					<?php $deliver_n += $product->actual_delivery_price;
						$delivery_breakup.="<table><tr><td>".IdController::nP($product->id)."</td><td>{{$currentCurrency}}&nbsp;<span id='".$product->id."'>".number_format($product->delivery_price,2)."</span></td></tr></table>";
					 ?>
			...
			<table class="table table-bordered cart-table" style="margin-top:20px">
				...
				?>
					...
					<tr >
						...
						<td style="border:none;">{{$currentCurrency}}</td>
						<td class="totalPrice"
					...
					<tr>
						<td class="summary" style="border:none;">Gross Total 
							<br>(Including {{$gst_tax_rate}}% GST)</td>
						<td style="border:none;">{{$currentCurrency}}</td>
				...
				<tr>
					...
					<td colspan="5">
						 <p id="fpx_error_message_amount" style="visibility: hidden;" class="text-danger pull-left">Minimum amount for FPX transaction is {{$currentCurrency}} 1.00 and Maximum amount is {{$currentCurrency}} 30,000.00</p>
					...
					<td colspan="2">
						...
						<p class="summary text-right">Total&nbsp;{{$currentCurrency}}&nbsp;
						...
						<p align="summary " class="text-right"
							...
							Final&nbsp;Total&nbsp;{{$currentCurrency}}&nbsp;<span class='totalPrice totalPrice2'>
							{!! number_format($grandtotal, 2) !!}</span></p>				
					</td>
				</tr>
			</table>
			...
			$.each(data.delivery,function(index,value){
				...
				for (var i = value.length - 1; i >= 0; i--) {
					content+="<tr><td>"+value[i][0]+
						"&nbsp;</td><td>{{$currentCurrency}}&nbsp;"+value[i][1]+"</td></tr>";
				}
				content+="</table>";
            				...
            				success:function(r){
            					if (r.status=="success") {
            						...
            						if (r.short_message==1 || r.short_message==-3 || r.short_message==-2 || r.short_message==-11) {
            							...
            							// zxcv
            							$('#vbank').attr('disabled',false);
            							$('.pmode').removeClass('grayout');
            							$('#vcard').addClass('grayout');
            							$('#vpaypal').addClass('grayout');
            							$('#other_banks').attr('disabled',false);
		...
		success:function(r){
			...
			if (r.status=="success") {
				...
				for (key in r.fpx_form) {
					fpx_html+="<input type='hidden' name='"+key+
					"' value='"+r.fpx_form[key]+"' >";
				};
				// console.log(r.fpx_form.fpx_txnAmount);
				if (r.fpx_form.fpx_txnAmount>30000 ||
					r.fpx_form.fpx_txnAmount <1) {
