				    	...
				    	{{-- Calculations --}}
				    		<?php
                                $qty = explode(',', $product['pledged_qty']);
                             	...
                             	$left_pledge = ($product->moq - $v) > 0 ? ($product->moq - $v) : 0; 
				    		 ?>
				          	{{-- Calculations --}}
						          	<?php
                                    $date = $product->odate;
                                   ...
                                   $left = ($product->moq - $v) > 0 ? ($product->moq - $v) : 0; 
								   if($left < 0){
									   $left = 0;
								   }
									...
									?> 									
						          	...
						          	<div class="line ">
  						          	<table class="table striped replacable_1">
  						          		<tr>
  						          			<td>MOQ</td>
  						          			<td class="text-center">{{$product->moq}}</td>
                              <td class="text-right">{{$currentCurrency}} {{number_format($price * $product->moq,2)}}</td>
  						          		</tr>
                            <tr>
                              <td>MOQ/location</td>
                              ...
                              <td class="text-right">{{$currentCurrency}} {{number_format($left * $price,2)}}</td>
											...
											<?php 
												$pl = 0;
												if($product->moq > 0 && $v > 0){
													$pl = ($v * 100)/$product->moq;
												}												
											?>
