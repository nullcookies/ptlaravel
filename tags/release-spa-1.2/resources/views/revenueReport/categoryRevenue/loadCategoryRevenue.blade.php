...
$(document).ready(function() {
	...
	var options = {
	    ...
	    title: {
	        text: 'Category Revenue Pie- {{ date("Y")}}'
	    },
	    tooltip: {
	        formatter: function() {
	...
	$.getJSON("{{ URL::to('/')}}/categoryRevenueJSON/{{$year}}/{{$month}}/json", function(json) {
	...
	<table id="revenueDatatable" class="display table table-bordered" cellspacing="0" width="100%">
        ...
        <tfoot>
            <tr>
                ...
                <td style="padding-right:0" class="text-right">{{$currentCurrency}}&nbsp;{{ number_format($totalCommission,2) }}</td>
...
<script type="text/javascript">
    ...
    $('#revenueDatatable').DataTable({
		...
		"aoColumnDefs": [
			/* Column #0 and column #1 has no sort function & no icons */
