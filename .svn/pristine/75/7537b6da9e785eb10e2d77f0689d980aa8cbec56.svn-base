...
//        dd($openwish_product_id);
  ...
  public function productsales($uid=null){
	...
	$channel = DB::table('fairlocation')->
		where('user_id', $user_id)->get();

	$data['categories'] =
		Category::join('product','product.category_id','=', 'category.id')->
		join('merchantproduct as mp','mp.product_id','=','product.parent_id')->
		where('mp.merchant_id',$merchant_id)->
		lists('category.description','category.id')->
		all();

	$data['brands'] =
		Brand::join('product','product.brand_id','=','brand.id')->
		join('merchantproduct as mp','mp.product_id','=','product.parent_id')->
		where('mp.merchant_id',$merchant_id)->
		lists('brand.name','brand.id')->
		all();

	$data['products'] =
		Product::join('merchantproduct as mp','mp.product_id','=','product.id')->
		where('mp.merchant_id',$merchant_id)->
		lists('product.name','product.id')->
		all();

	$data['countries'] = $this->countryModel->
		where('code','=','MYS')->
		lists('name','id')->
		all();

	return view('merchant.productsales',
		compact('since','channel','data'));
  } 
  ...
  public function staffsales($uid = null){
    ...
    $channel = DB::table('fairlocation')->where('user_id', $user_id)->get();
    return view('merchant.staffsales',compact('since','channel'));
  }      
