				...
				<div class="col-md-12" style="">
					<form id="notes-form">
						<fieldset>
							...
							<input type="hidden" id="copy" value="0">
						</fieldset>
					</form>
				</div>
			</div>
		...
		<table class="table table-bordered" cellspacing="0" width="100%" id="gridmerchant">
			<thead style="background-color: #FF4C4C; color: white;">
				<tr>
					...
					<th class="text-center bsmall" style="width: 220px !important">Select</th>
				</tr>
			</thead>
			<tbody>
				<?php $m = 0; ?>
				...
				<tr style="{{$style}}">
					...
					<td class="text-center" style="width: 220px !important">
				...
				<tfoot>
					<tr>
						...
						@if($oshop->status == 'transferred')
							<th style="width: 220px !important"><button class='btn btn-block btn-danger btn-sm btn-transfer' disabled rel="{{$oshop->id}}">Transfer</button></th>
						@else
							<?php 
							...
							@if($countp == 0)
								<th style="width: 220px !important">
									<p align="center">
										<button class='btn btn-danger btn-sm btn-transfer' style="width:120px" title='This O-Shop have no product to be transferred' disabled rel="{{$oshop->id}}">Transfer</button>
										<button class='btn btn-info btn-sm btn-copy' style="width:120px" title='This O-Shop have no product to be copied' disabled rel="{{$oshop->id}}">Copy</button>
								</th>
							@else
								<th style="width: 220px !important">
									<p align="center">
										<button class='btn btn-danger btn-sm btn-transfer' style="width:120px" rel="{{$oshop->id}}">Transfer</button>
										<button class='btn btn-info btn-sm btn-copy' style="width:120px" rel="{{$oshop->id}}">Copy</button>
									</p>
								</th>
							@endif
						@endif
...
$(document).ready(function () {
    ...
    function pad (str, max) {
        ...
        return str.length < max ? pad("0" + str, max) : str;
			...
			$(document).delegate( '.btn-copy', "click",function (event) {
				$("#tproduct_id").val($(this).attr('rel'));
				$("#myModalNotes").modal('show');
				$("#copy").val(1);
			});
				...
				if(oshop == ""){
					...
					$.ajax({
						...
						success: function (data) {
							...
							if(copyvv == 1){
								toastr.info('O-Shop Successfully Copied');
							} else {
