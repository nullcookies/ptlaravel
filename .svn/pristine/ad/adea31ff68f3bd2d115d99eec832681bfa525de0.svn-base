	...
	<div class="table-responsive">
		<table id="myTable" class="table table-bordered myTable">
			<thead style="background-color: #FF8B91; color: #fff;">
				...
				<th class="text-center">Retail&nbsp;Commission</th>
			</thead>
			<tbody>
				@def $i = 0
				@foreach($merchantscom as $merchantsc)
					...
					<tr>
						...
						<td class="text-center"><a href="http://{{$_SERVER['SERVER_NAME']}}/album/{{$merchantsc->pid}}">{{IdController::nP($merchantsc->pid)}}</a></td> 
						<td>{{$merchantsc->pname}}</td>
						<td class="text-center">
							@if($merchantsc->osmall_commission > 0)
								<p id="mv_p{{$merchantsc->pid}}" class="regp_edit" rel="{{$merchantsc->pid}}">{{number_format($merchantsc->osmall_commission,2,'.','')}}%</p><p id="mv_i{{$merchantsc->pid}}" style="display:none;"><input type="text" class="regp_input" rel="{{$merchantsc->pid}}" value="{{number_format($merchantsc->osmall_commission,2,'.','')}}" id="mv_c{{$merchantsc->pid}}" size="2"/>%</p>
							@else
								<p id="mv_p{{$merchantsc->pid}}" class="regp_edit" rel="{{$merchantsc->pid}}">{{number_format($global_vars->osmall_commission,2,'.','')}}%</p><p id="mv_i{{$merchantsc->pid}}" style="display:none;"><input type="text" class="regp_input" rel="{{$merchantsc->pid}}" value="{{number_format($global_vars->osmall_commission,2,'.','')}}" id="mv_c{{$merchantsc->pid}}" size="4"/>%</p>	
							@endif
						</td>
					</tr>					
				@endforeach
...
$(document).ready(function () {
	...
	//$('.mv_btnedit').click(function(){
		...
		if($.isNumeric(commission)){
			var url = '/admin/commission/productedit/'+ mv_id;
			$.ajax({
			  url: url,
			  type: "post",
