...
var CanvasRenderer = function () {
	...
	_createClass(CanvasRenderer, [{
		...
		value: function prepareCanvas() {
			...
			if (this.options.price!=undefined && this.options.price!="") {
				this.canvas.height+=this.options.fontSize+this.options.textMargin;
			}
			console.log("Canvas Height",this.canvas.height);
			...
			// Paint the canvas
			ctx.clearRect(0, 0, this.canvas.width, this.canvas.height+100);
			if (this.options.background) {
				ctx.fillStyle = this.options.background;
				/* To Change Background of Barcode*/
		...
		value: function drawCanvasText(options, encoding) {
			...
			if (options.displayValue) {
				var x, y,xPrice,yPrice,xDesc,yDesc;
				console.log("drawCanvasText");

				x=0;
				yDesc=0;
				y = options.height + options.textMargin + options.marginTop + options.fontSize+options.fontSize;
				yPrice=y+options.fontSize;

				ctx.font = font;
				console.log("x",x,"y",y,"xPrice",xPrice,"yPrice",yPrice,"xDesc",xDesc,"yDesc",yDesc);
